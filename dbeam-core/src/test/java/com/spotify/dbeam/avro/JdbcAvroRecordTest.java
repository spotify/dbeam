...
import static com.spotify.dbeam.avro.AvroSchemaMetadataProviderTest.createProvidedSchema;
import static com.spotify.dbeam.jobs.InputAvroSchemaTest.getMockConnection;
...
public class JdbcAvroRecordTest {
  ...
  @Test
  public void shouldCreateSchemaWithUserProvidedSchemaAndBackupDocForMissingField()
      throws SQLException {

    List<String> fieldNames = Arrays.asList("dummyColumnName0", "dummyColumnName1");
    Schema providedSchema = createProvidedSchema(fieldNames);

    AvroSchemaMetadataProvider provider =
        new AvroSchemaMetadataProvider(
            providedSchema, "Default schema name", "Default schema namespace", "Default avro doc");

    Connection mockConnection = getMockConnection(3);
    final Schema actualSchema =
        JdbcAvroSchema.createSchemaByReadingOneRow(
            mockConnection, QueryBuilderArgs.create("dummy_table"), false, provider);

    // value provide by a user-supplied schema
    Assert.assertEquals(
        "Doc for dummyColumnName0", actualSchema.getField("dummyColumnName0").doc());
    // value provide by a user-supplied schema
    Assert.assertEquals(
        "Doc for dummyColumnName1", actualSchema.getField("dummyColumnName1").doc());
    // back-up value generated by default
    Assert.assertEquals(
