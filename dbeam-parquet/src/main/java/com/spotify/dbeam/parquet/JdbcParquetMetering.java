...
public class JdbcParquetMetering {
  ...
  private final Logger logger = LoggerFactory.getLogger(JdbcParquetMetering.class);
  ...
  public JdbcParquetMetering(int countReportEvery, int logEvery) {
    this.countReportEvery = countReportEvery;
    this.logEvery = logEvery;
  }

  public static JdbcParquetMetering create() {
    return new JdbcParquetMetering(100000, 100000);
  }
  ...
  public void incrementRecordCount() {
    ...
    if ((this.rowCount % countReportEvery) == 0) {
      ...
      if ((this.rowCount % logEvery) == 0) {
        logger.info(
            String.format(
                "jdbcparquetio : Fetched # %08d rows at %08d rows per minute"
                    + " and %08d ms per M rows",
  ...
  public void exposeWriteElapsed() {
    ...
    logger.info("jdbcparquetio : Read {} rows, took {} seconds", rowCount, elapsedMs / 1000.0);
  ...
  public void exposeExecuteQueryMs(final long elapsedMs) {
    logger.info("jdbcparquetio : Execute query took {} seconds", elapsedMs / 1000.0);
